...
fn qft_bench(c: &mut Criterion) {
	...
	for backend in backends.iter() {
		for nb_qubits in 2..=6 {
			...
			let circuit = Circuit::from_matrix(qft_matrix(nb_qubits));
...
fn ghz_n_bench(c: &mut Criterion) {
	...
	for backend in backends.iter() {
		for nb_qubits in 2..=8 {
			...
			let mappings: Vec<_> = (0..=nb_qubits).collect();
			let mut circuit = Circuit::new(nb_qubits + 1).then(Gate::map(&Gate::h(), &mappings));
